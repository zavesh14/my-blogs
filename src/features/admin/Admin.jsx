...
function normalizeImageUrl(value) {
  ...
  const driveFile = url.match(/drive\.google\.com\/file\/d\/([^/]+)/i) || url.match(/[?&]id=([^&]+)/i)
  return driveFile ? `https://drive.google.com/uc?export=view&id=${driveFile[1]}` : url
}
...
export default function Admin() {
  ...
  <section className="admin-panel gallery-panel" id="gallery"><div className="panel-heading"><h2>Edit gallery</h2><button className="subtle-button" onClick={addPhoto}><Plus size={14} /> Add photo</button></div>{photos.map((photo) => <div className="gallery-admin-card" key={photo.id}><label className="admin-label">Photo URL<input type="url" placeholder="https://... or Google Drive share link" value={photo.src || ''} onChange={(e) => update(setPhotos, photos, photo.id, 'src', normalizeImageUrl(e.target.value))} /></label>{photo.src && <img className="admin-image-preview gallery-preview" src={normalizeImageUrl(photo.src)} alt="Gallery preview" />}<label className="admin-label">Caption<input value={photo.label || ''} onChange={(e) => update(setPhotos, photos, photo.id, 'label', e.target.value)} /></label><label className="admin-label">Alt text<input value={photo.altText || ''} onChange={(e) => update(setPhotos, photos, photo.id, 'altText', e.target.value)} /></label><label className="admin-label"><input type="checkbox" checked={Boolean(photo.published)} onChange={(e) => update(setPhotos, photos, photo.id, 'published', e.target.checked)} /> Published</label><div className="admin-actions"><button type="button" className="save-button" onClick={() => saveContent('gallery', photo)}><Save size={15} /> {savedItem === `Photo "${photo.label}" saved` ? 'Saved' : 'Save photo'}</button>{savedItem.startsWith(`Photo "${photo.label}"`) && <span className="save-confirmation">{savedItem}</span>}<button className="delete-button" onClick={() => removeContent('gallery', photo)}><Trash2 size={14} /> Delete</button></div></div>)}</section>
